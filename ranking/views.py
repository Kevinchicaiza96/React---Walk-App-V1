...
# ---------------------------------
# VISTA PRINCIPAL DE RANKING (Django template - no cambia)
# ---------------------------------
    ...
    if request.user.is_authenticated:
        try:
            user_profile, created = UserProfile.objects.get_or_create(
                user=request.user,
                defaults={'total_puntos': 0, 'Puntos_mensuales': 0, 'puntos_semanales': 0, 'distancia_total_km': 0, 'dias_activos': 0}
            )
            user_walks = Walk.objects.filter(usuario=request.user).order_by('-fecha')[:3]
            chart_data = {'Semana Actual': [user_profile.puntos_semanales, round(user_profile.distancia_total_km, 2), user_profile.dias_activos]}
...
# ---------------------------------
# API: ESTADÍSTICAS DEL USUARIO (JWT)
# ---------------------------------
@api_view(['GET'])
@permission_classes([IsAuthenticated])
...
# ---------------------------------
# API: ACTUALIZAR POSICIÓN EN TIEMPO REAL (JWT)
# ---------------------------------
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def api_actualizar_posicion(request):
    try:
        ...
        lat = data.get('lat')
        lng = data.get('lng')
        distancia_metros = data.get('distancia_metros', 0)
