...
# ===========================
# MODELO: COMENTARIO DE RUTA
# ===========================

class ComentarioRuta(models.Model):
    ...
    usuario = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='comentarios_rutas',
    ...
    class Meta:
        ...
        ordering = ['-fecha']
        unique_together = ('ruta', 'usuario')  # Un comentario por usuario por ruta

    def __str__(self):
        return f"{self.usuario.username} → {self.ruta.nombre_ruta} ({self.estrellas}⭐)"